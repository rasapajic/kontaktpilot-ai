  ...
  return (
    <div style={{ minHeight:'100dvh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', background:'var(--bg)', padding:'24px 20px' }}>
      ...
      <div style={{ width:'100%', maxWidth:420 }}>
        <h1 className="t-section" style={{ color:'var(--ink)', textAlign:'center', marginBottom:6 }}>Welcome back.</h1>
        <p style={{ textAlign:'center', color:'var(--ink-2)', marginBottom:28, fontSize:'1rem' }}>Sign in to your account</p>

        <div className="card" style={{ padding:'32px 28px' }}>
          {error && (
            <div className="status-red" style={{ borderRadius:'var(--r)', padding:'14px 16px', marginBottom:20 }}>
              <p style={{ color:'var(--red)', fontSize:'.95rem', fontWeight:500 }}>{error}</p>
            </div>
          )}
          <form onSubmit={submit} style={{ display:'flex', flexDirection:'column', gap:16 }}>
            ...
            <div>
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:8 }}>
                <label style={{ fontSize:'.78rem', fontWeight:700, letterSpacing:'.08em', textTransform:'uppercase', color:'var(--ink-3)' }}>Password</label>
                <a href="#" style={{ fontSize:'.85rem', color:'var(--gold-4)', textDecoration:'none', fontWeight:500 }}>Forgot?</a>
              </div>
              <input type="password" required value={pass} onChange={e=>setPass(e.target.value)} placeholder="Your password" className="input" />
            </div>
        ...
        <p style={{ textAlign:'center', marginTop:20, fontSize:'.95rem', color:'var(--ink-2)' }}>
          No account? <Link href="/register" style={{ color:'var(--ink)', fontWeight:600, textDecoration:'none' }}>Create one free →</Link>
        </p>
      </div>
    </div>
  )
